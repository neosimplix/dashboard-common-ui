...
0.5.0 의 레일 재설계가 **세우는 쪽을 없앴다.** 레일은 이제 최상위 그룹의 타일만 그리고 항목도 라벨도 그리지 않으므로, "레일에서 라벨을 숨긴다"·"레일에서 접힘을 무시한다" 는 상황 자체가 존재하지 않는다. 두 이름을 세우던 `::slotted(ns-nav-group)` 규칙 둘이 사라졌다.
...
**0.5.0 이 두 이름을 함께 지웠다.** 레일이 최상위 그룹의 타일만 그리게 되어 "레일에서는 접힘을 무시해 항목에 도달할 경로를 남긴다" 는 전제가 사라졌다 — 레일에는 타일만 있고, 그룹에 도달하는 경로는 타일이다. `.list.collapsed` 는 `display: none` 리터럴로 돌아갔다. 위 "접힘 전파…" 절에 경위가 있다.
...
`ns-sidebar` 의 레일은 그룹마다 키가 필요하다 — 타일의 `id`, `aria-controls`, `activeGroup` 이 가리키는 값, 아이콘을 짝지을 이름이 전부 그 하나다. 이름 후보가 `key` 였다.
...
→ **키는 `name` 이다.** HTML 전역 속성이 아니고 `HTMLElement.prototype.name` 도 없으므로(`name` 을 가진 것은 `input`·`form`·`iframe` 등 특정 요소들이다) 덮을 것이 없다. React 도 특별하게 다루지 않는다.

→ 일반화하면, **React 가 프롭 이름을 예약해 둔 것은 shim 으로 우회할 수 없다.** `key`·`ref` 가 그렇다. HTML 전역 속성(`title`·`style`·`id`)은 도착하므로 우회할 수 있다. **두 목록을 같은 것으로 취급하면 "shim 이 있으니 괜찮다" 로 잘못 판정한다.**

## 수동 슬롯 배정은 소비자 DOM 을 관리하지 않으려고 고른 것이다

0.5.0 의 `ns-sidebar` 는 패널에 **선택된 그룹 하나만** 보여준다. 자동 배정이면 그룹 전부가 슬롯에 들어오므로 선택되지 않은 것을 숨겨야 한다.
...
**자동 배정 + 숨김의 대가가 이 선택의 근거다.** `::slotted()` 는 결합자를 받지 않으므로 "선택되지 않은 그룹" 을 상태로 고를 수단이 없고, 결국 소비자 DOM 에 표시 속성(`data-ns-panel-hidden` 같은 것)을 써야 한다. 그러면 **이 저장소가 이미 값을 지불한 것 둘이 함께 온다** — 소비자 DOM 의 속성을 관리하면 `MutationObserver` 가 필요하고(`updated()` 는 남의 DOM 변경을 보지 못한다), 문서 이름공간의 훅 이름은 충돌하면 엉뚱한 요소를 오인해 에러 없이 오동작한다.

수동 배정은 그 둘을 피하면서 셋을 더 준다.
...
- **공백 텍스트 노드가 무해하다.** 자동 배정에서는 사이드바 자식의 줄바꿈이 기본 슬롯으로 가서 패널에 들어가는데(위 "기본 슬롯의 폴백은 공백 텍스트 노드가 죽인다" 와 같은 알고리즘이다), 수동 배정에서는 우리가 배정한 것만 렌더된다.
...
### 대가 — `slot` 속성이 아무 의미가 없어진다

배정을 전부 우리가 하므로 브라우저는 `slot` 속성을 보지 않는다. 그래서 레일 아이콘의 표시가 `slot="rail-admin"` 이 아니라 **`data-ns-rail="admin"`** 이다.
...
`slot=` 을 그대로 쓰면 **동작하지 않는 표준 속성**이 되어 더 나쁘다 — 소비자는 그것이 표준 슬롯 배정이라고 읽고, 오타를 냈을 때 브라우저가 알려줄 것이라고 기대한다. 같은 이유로 shadow 안의 `<slot>` 에 `name` 을 주지 않는다. 수동 배정에서는 이름 매칭이 일어나지 않으므로 `name="rail-admin"` 은 동작하지 않는 장식이다. 대상을 찾을 때 쓰는 표시는 `data-name` 이고, 그것은 shadow 안이라 문서 이름공간과 충돌하지 않아 접두사가 필요 없다.

### `data-ns-rail` 에는 정적 검사가 없다 — 규칙 ③ 을 넓히지 않는다

`check-tokens.mjs` 의 규칙 ③ 은 `data-ns-*` 훅 이름을 세 층에서 대조한다 — `src/react/tags/*.tsx`(shim 이 쓰는 자리) · `src/tokens/tokens.css`(문서 예약이 읽는 자리) · `src/components/**/*.styles.ts`(shadow 가 읽는 자리). **그 셋은 CSS 가 짝인 훅이 걸치는 층이다.** `data-ns-rail` 은 CSS 가 아니라 **코드가 질의하는** 훅이라 셋 중 어디에도 나타나지 않고, 그래서 애초에 수집되지 않는다.
...
→ **규칙을 넓히지 않는다.** 넓히려면 "훅마다 필요한 층" 을 파일에 명시하는 쪽으로 가야 하고(같은 한계 목록이 그 방향을 이미 적어 두었다), 지금 그것을 지불할 이유는 훅 하나다. 대신 **이름이 어긋났을 때의 증상을 육안 확인 항목으로 옮긴다** — 타일이 아이콘 대신 폴백 글자를 보여주는 것뿐이라 콘솔도 조용하고 화면도 멀쩡해 보인다.

## 레일 아이콘은 그룹이 아니라 사이드바의 자식이어야 한다

레일 타일이 두 자리에 나뉘는 마크업(아이콘은 사이드바 자식, 제목·배지는 그룹)은 한눈에 어색하다. 자연스러운 쪽 둘이 **막혀 있다.**

**그룹이 자기 타일을 그리는 길.** 타일들은 패널 내용의 높이와 무관하게 한 컨테이너에 세로로 쌓여야 하는데, 그룹의 shadow 안에서 그린 것은 그 그룹 호스트의 자리에 묶여 있다. 두 번째 그룹의 타일이 첫 그룹의 패널 내용 아래로 밀린다. `:host { display: contents }` 로 사이드바의 grid 에 참여시켜도 행이 짝지어져 같은 문제가 남고, 절대 위치로 띄우면 세로 쌓기를 JS 로 계산해야 한다. → **타일은 사이드바의 shadow 가 그리고, 타일에 들어갈 것은 사이드바가 읽을 수 있는 데이터여야 한다.**
...
`<ns-nav-group icon="users">` 는 기본 경로다. 그룹의 정의가 마크업 한 자리에 모이고, 스프라이트는 `registerIcons()` 로 열려 있다. 그런데 그것만으로 안 되는 경우가 둘이다.
...
→ 그래서 경로가 둘이다. **SVG 문자열을 프로퍼티로 받는 셋째 길은 쓰지 않는다** — `unsafeSVG` 가 들어오고, 이 저장소가 명령형 API 셋을 "문자열만 받으므로 HTML 주입 경로가 없다" 로 정당화한 것과 어긋난다. `registerIcons` 가 이미 그 자리를 정당한 방식으로 채우고 있다.
...
**4단이 있는 이유는 이주다.** 0.4.0 소비자의 그룹에는 `heading` 만 있으므로, 마크업을 하나도 고치지 않아도 레일이 빈 타일이 되지 않는다. 1~3 은 "둘 중 하나만 보여야 하는 자리는 슬롯 폴백으로 만든다" 규칙 그대로이고 — 프로퍼티 넷으로 소비자가 분기하게 하지 않는다 — **수동 배정이라 `ns-icon` 이 겪은 공백 텍스트 노드 문제가 없다.**

→ **문서가 할 수 있는 최선은 배치를 요구하는 것이다.** 아이콘을 그 그룹 바로 앞에 두라고 적는다. 짝이 국소적으로 읽히게 하는 것 말고, 마크업 두 자리를 하나로 되돌릴 방법은 위 두 이유 때문에 없다.
...
## 최상위 그룹의 `collapsible` 은 무르지 않는다

패널에는 선택된 그룹 하나만 온다. 그 하나가 `collapsible` 이고 접히면, 패널은 통째로 빈 채로 보인다 — 레일 타일은 남아 있는데 눌러도 아무것도 나오지 않는 것처럼 보인다.

라이브러리가 이 조합을 판정 정보(그룹이 최상위인지)로 감지해 `collapsible` 을 무시하게 만들 수는 있다. **하지 않는다.** `.claude/rules/library-invariants.md` 의 "제어 중이면 그 값을 바꾸지 않는다" 와 같은 결의 원칙이 여기에도 있다 — **소비자가 명시적으로 요청한 것을 라이브러리가 판단해 무르지 않는다.** 0.4.0 부터 `collapsible` 을 최상위 그룹에 써 온 소비자가 있다면(레일이 없던 시절에는 문제가 없었다), 0.5.0 에서 그 그룹이 우연히 사이드바의 유일한 최상위 그룹이 되는 순간 조용히 동작이 바뀌는 것이 오히려 더 나쁘다.

→ 그래서 처방은 문서다. `collapsible` 은 하위 그룹에 쓰라고 적고, 최상위에 쓰면 무엇이 보이는지(패널이 빈다) 도 함께 적는다. `index.html` 의 `ns-nav-group` 절과 `docs/project-structure.md` 가 그 자리다.

## 닫힌 사이드바에는 `navigation` 랜드마크가 하나도 없다

`ns-sidebar` 의 `<nav class="panel">` 이 이 컴포넌트의 유일한 `nav` 이고, 패널이 닫히면 `:host(:not([data-ns-open])) .panel { display: none }` 이 걸린다. 레일은 `.shell` 의 자식이라 그 `nav` **밖**에 있다. 0.4.0 은 접힘 상태에서도 항목이 `nav` 안에 있었으므로 **이것은 0.5.0 이 잃은 것이다.**

`role="tabpanel"` 을 `nav` 에 얹지 않기로 한 결정(같은 파일의 설계 §3, `ns-sidebar.ts` 의 `render()` 주석)이 "네비게이션 사이드바에서 랜드마크는 잃어도 되는 것이 아니다" 를 근거로 삼는데, 닫힘 상태에서 그것이 그대로 사라진다. 근거를 적어 둔다.

**받아들인 절충이다.** 고치는 길은 랜드마크를 `.shell` 로 올려 레일까지 그 안에 넣는 것인데, 그러면 `navigation` 랜드마크가 `tablist` 를 품고 그 안에 또 하나의 영역이 들어가는 모양이 된다 — 랜드마크의 이름과 범위가 "지금 보이는 네비게이션" 이 아니라 "레일 + (있으면) 패널" 로 바뀌므로, 접근성 트리의 모양을 바꾸는 변경이다. 그에 비해 잃는 것은 작다.

- **탐색 경로가 사라지지 않는다.** 닫힌 사이드바에서 보이는 것은 레일뿐이고, 레일은 `role="tablist"` 에 타일마다 `role="tab"` + `aria-label` 이라 스크린 리더가 이름 있는 구조로 읽는다. 랜드마크 목록에서 사라질 뿐 도달 불가능해지지 않는다.
- **닫힘은 사용자가 만든 상태다.** 사용자가 방금 접은 영역이 랜드마크 목록에서 빠지는 것은 `display: none` 인 영역의 정상적인 결과이고, 대화상자가 닫혔을 때와 같은 종류다.
- **셸에는 다른 랜드마크가 있다.** 이 저장소의 `index.html` 과 소비자 셸 모두 `<main>` 을 갖는다. 랜드마크가 0 이 되는 것이 아니라 `navigation` 하나가 그 상태에서 없는 것이다.

다시 판단할 계기가 생기면(소비자가 랜드마크 점프로 사이드바를 찾는다고 보고하면) 처방은 위의 "`.shell` 로 올리기" 이고, 그것은 접근성 트리를 바꾸므로 별도 릴리스로 낸다.
