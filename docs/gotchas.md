...
→ **레일을 물리면서 그 조합 자체가 정상 동작이 됐다.** 사이드바가 최상위 그룹을 전부 그리므로 하나를 접어도 나머지가 그대로 보인다. 오히려 **긴 네비게이션에서는 최상위에 쓰는 것이 목록을 가장 크게 줄이므로 주 용도에 가깝다** — 이 저장소의 `index.html` 좌측 네비게이션이 그렇게 돼 있다. 한동안 문서 곳곳에 있던 "`collapsible` 은 하위 그룹에 쓴다" 는 안내는 레일 시절의 것이고 지금은 반대다.
...
- **셸에는 다른 랜드마크가 있다.** 이 저장소의 `index.html` 과 소비자 셸 모두 `<main>` 을 갖는다. 랜드마크가 0 이 되는 것이 아니라 `navigation` 하나가 그 상태에서 없는 것이다.
