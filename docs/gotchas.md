...
→ **경고할 수 있다.** 이 이름의 속성이 호스트에 붙는 경로가 하나뿐이기 때문이다. 라이브러리는 호스트에 속성을 쓰지 않고(불변 규칙), Lit 은 `attribute: false` 를 반영하지 않으며, React shim 은 프로퍼티로 넘긴다. **따라서 발견되면 언제나 소비자가 쓴 것이고 언제나 실수다.** 판정에 애매함이 없으므로 거짓 경고가 나올 수 없다.
