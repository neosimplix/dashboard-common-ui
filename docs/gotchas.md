...
**판정을 자기 프로퍼티 둘로만 잡은 이유.** 요청서의 원안(문서에 `ns-header` 가 있으면 경고)과 회신의 대안(`ns-toggle` 을 한 번 수신했는데 그 뒤로 `open` 이 설정된 적이 없으면 경고) 둘 다 결국 교차 검사로 되돌아간다 — `ns-toggle` 은 `ns-header` 만 낸다(`src/components/header/ns-header.ts:60`). 사이드바가 그것을 알려면 문서에서 버블링 이벤트를 주워야 하고, 그러면 "어느 헤더가 어느 사이드바를 겨냥하는가, 헤더가 나중에 업그레이드되면 어떻게 되는가" 가 이름만 바꿔 돌아온다. 대신 `open === undefined && !hasAttribute("default-open")` 하나만 본다 — 그 조합이면 `#isOpen`(`this.open ?? this.#innerOpen`)이 영원히 `false` 라 타이머도 다른 컴포넌트도 사용자 상호작용도 열 방법이 없다는 것을 자기 상태만으로 증명할 수 있다.

**시점을 매크로태스크(`setTimeout(…, 0)`)로 미룬 이유 — 이 파일의 두 번째 피해자.** `willUpdate` 위 주석이 이미 같은 함정을 적어 두었다: `customElements.define` 이 `hydrateRoot` 보다 먼저 실행되므로 첫 업데이트의 마이크로태스크가 하이드레이션 커밋의 `useLayoutEffect` 보다 먼저 흐르고, `@lit/react` 의 `createComponent` 는 반응형 프로퍼티를 그 `useLayoutEffect` 에서만 설정한다. `connectedCallback` 이나 `firstUpdated` 에서 즉시 재면 React 소비자가 **전부** 거짓 양성이 된다 — `open` 을 나중에 세울 참인데 아직 세우지 못했을 뿐인 상태를 "영원히 못 연다" 로 오판한다. 매크로태스크로 미루면 그 사이 창을 건너뛴다.

**프로브를 만들다가 걸린 진짜 함정 — 별도 `<script>` 태그가 매크로태스크보다 늦게 실행된다는 보장은 없다.** 처음 짠 프로브는 `<script src="dist/bundle.umd.js">` 로 라이브러리를 불러온 뒤, 별도의 `<script type="module">` (또는 별도의 classic `<script>`) 안에서 `document.getElementById("c").open = false` 로 "React 가 나중에 프로퍼티를 세우는 상황"을 흉내 냈다. 실제 브라우저(헤드리스든 아니든, `--virtual-time-budget` 유무와 무관하게 CDP 로 실시간 콘솔을 들어보고 확인했다)에서 이 구성은 **매번 거짓 양성을 냈다** — `c` 도 함께 경고했다. 이유는 `<script src>` 실행이 끝나고 다음 `<script>` (module 이든 classic 이든)로 넘어가는 지점에 실제 태스크 경계가 생기고, `connectedCallback` 이 예약한 `setTimeout(…, 0)` 이 그 경계에서 먼저 처리되기 때문이다. **그런데 실제 소비자 앱에는 이 경계가 없다** — 번들러(webpack/vite)가 라이브러리 등록 코드와 `ReactDOM.hydrateRoot` 호출을 **하나의 스크립트**로 묶으므로, `customElements.define` 과 그 안의 동기적 `useLayoutEffect` 커밋은 같은 실행 구간 안에서 순서대로 끝나고, 그 뒤에야 이벤트 루프가 매크로태스크 큐로 넘어간다. 그래서 프로브도 두 단계를 별도 `<script>` 태그로 나누지 않고, 번들 코드 바로 뒤에 `open` 대입 코드를 **같은 스크립트 안에** 이어 붙여야 한다 — 나누면 실제 소비자에게는 없는 경계를 만들어서, 실제로는 안전한 타이밍을 위험한 것으로 오판하게 된다. `docs/gotchas.md` 의 "도구가 다른 목적의 문자열/타이밍을 읽는다" 계열과 같은 뿌리다 — 이번엔 문자열이 아니라 **테스트 하네스가 실제로는 없는 태스크 경계를 만들어 낸 것**이 문제였다.

**프로브의 `c` 케이스가 진짜 바인 이유.** `a`(아무 것도 없음, 경고 나야 함), `b`(`default-open`, 경고 없어야 함), `c`(같은 스크립트 안에서 정의 직후 `open` 을 프로퍼티로 세움 — React 소비자 흉내, 경고 없어야 함) 셋을 같은 페이지에 두고, 위 방식대로 하나의 `<script>` 로 합쳐 CDP 로 실측하면 총 경고 수는 정확히 1(`a` 만)이다. 조건을 일부러 뒤집어(`this.open !== undefined && …`) 같은 프로브를 다시 돌리면 총 개수는 똑같이 1 이지만 **경고하는 대상이 `a` 에서 `c` 로 바뀐다** — 진짜 배선 실수는 조용해지고, 정상적으로 배선된 React 소비자가 대신 경고를 받는다. 총 개수만 보면 이 회귀를 놓친다는 뜻이라, 다음에 이 프로브를 재사용할 때는 메시지에 요소 `id` 를 임시로 찍어서 **어느 요소가 경고했는지**까지 확인해야 한다.
