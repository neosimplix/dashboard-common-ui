...
→ **무승부를 깨는 것은 이름이 아니라 특정도다.** `:root[data-theme="light"]` 는 속성 선택자가 하나 더 붙어 `:root` 를 이기고, **이기는 이유가 순서가 아니므로 임포트 위치와 무관하다.** 그래서 소비자에게 시킬 것은 하나로 정해진다 — 자기 CSS 의 `color-scheme` 을 지우고 `data-theme` 으로 옮긴다. 그 속성이 결정적인 옵트아웃이라는 것을 `README.md` 와 `index.html` 이 각각 적는다.
...
`ns-icon` 에 기본 슬롯을 열면서 스프라이트를 그 폴백으로 두었다. `<ns-icon><House /></ns-icon>` 가 동작하고 `<ns-icon name="menu"></ns-icon>` 는 그대로다 — 분기가 슬롯 배정 규칙이라 우리가 셀 것이 없다. `ns-nav-item` 의 `leading`/`badge` 와 같은 형태다.

**그런데 두 경우가 같지 않다.** `leading` 은 명명 슬롯이다. `slot` 속성이 없는 노드는 기본 슬롯으로 가므로, 소비자가 태그 사이에 넣은 줄바꿈과 들여쓰기는 `leading` 에 배정되지 않는다. 폴백이 안전하다.
...
## 육안 확인의 매개체가 스스로 깨졌다 — `index.html` 의 리터럴 색

이 저장소의 회귀 확인은 둘이다. `npm run check` 와 `index.html` 육안 확인. 그런데 **두 번째 수단 자신이 조용히 깨질 수 있다는 것을 `check-tokens.mjs` 의 한계 주석이 이미 예고하고 있었다** — 그 파일은 배포되고 소비자가 열어 보는데 검사 대상이 아니라고. 0.2.4 에서 실제로 그 일이 일어났다.
...
→ **검사로 막을 수 있는 부분이 있었다.** `index.html` 전체에 규칙 ①을 켤 수 없는 이유는 산문이 옛 무접두사 이름을 일부러 적기 때문인데, **`<style>` 블록 안은 산문이 아니다.** 그 안의 선언만 보면 오탐이 없다. 규칙 ⑤ 가 그것이고, 리터럴 색이 하나도 없다는 것이 지금 기준선이다.
...
우회로 끝낼 수 없는 이유는 문서가 반대로 안내하고 있었기 때문이다. README·`index.html` 이 `lucide-react` 를 권하는데 그건 **항상** `width`/`height` 를 찍는다. 데모에도 그 성질을 보이려고 `width="40"` 을 단 아이콘을 넣어 두었는데, 그 아이콘이 로드할 때마다 40 → 20 으로 튀고 있었다. **문서 페이지가 자기가 설명하는 결함을 시연하고 있었다.**
...
→ **경고할 수 있다.** 이 이름의 속성이 호스트에 붙는 경로가 하나뿐이기 때문이다. 라이브러리는 **그 이름들을** 호스트에 쓰지 않고(불변 규칙 — 호스트에 쓰는 두 예외는 `role="tablist"` 와 `ns-toast` 의 `position` 뿐이고 둘 다 이 목록에 없다), Lit 은 `attribute: false` 를 반영하지 않으며, React shim 은 프로퍼티로 넘긴다. **따라서 발견되면 언제나 소비자가 쓴 것이고 언제나 실수다.** 판정에 애매함이 없으므로 거짓 경고가 나올 수 없다.
...
## 호스트에 속성을 쓰는 예외는 둘이고, 서로 종류가 다르다
...
**`ns-tabs` 와 같은 이유로 허용되는 것이 아니다.** tablist 는 *"그 속성이 호스트에 있어야만 의미가 성립한다"* 는 쪽이고, `ns-toast` 는 *"덮어 없앨 소비자 값이 존재하지 않는다"* 는 쪽이다. 이 태그는 소비자가 마크업에 쓰는 태그가 아니다 — `nsToast()` 가 만들어 `document.body` 에 붙이고 이후 재사용하며, `index.html` 이 "직접 마크업에 쓰지 않는다" 로 못박고 있다. **소비자가 쓴 적 없는 속성은 덮을 수도 없다.**
...
| 그 태그를 **소비자가 마크업에 쓰는 경로가 아예 없는가** (라이브러리가 만들어 붙이는 요소인가) | 덮을 값이 없으므로 조건 없이 쓴다 | `ns-toast` 의 `position` |
...
`index.html` 의 slot 표는 그것을 **"우측 정렬과 간격이 내장"** 이라고 문서화하고 있었다. 그런데 **그 간격은 어느 소비자에게도 도달한 적이 없다.** 세 사용처가 모두 버튼을 `<div slot="footer">` 로 감쌌기 때문이다 — `index.html` 데모, `confirm.ts`, React `Dialog` shim.
...
그래서 `nsConfirm` 의 취소·확인이 **서로 맞닿아** 그려졌다. `index.html` 데모가 멀쩡해 보인 것은 마크업의 줄바꿈이 인라인 공백 하나를 만들어 줬기 때문이고, 노드를 `append` 로만 붙이는 `confirm.ts` 에는 그 공백조차 없었다.
...
**이 결함은 이 저장소의 두 확인 수단 중 어느 쪽으로도 잡히지 않았다.** `npm run check` 는 CSS 선언의 유효성을 보지 값이 어떻게 해석되는지를 보지 않고, `index.html` 육안 확인은 **엔진 하나**로만 한다. 「브라우저 확인은 사람이 한다」 는 규칙이 암묵적으로 "사람이 쓰는 브라우저 하나" 를 뜻하고 있었던 셈이다.
