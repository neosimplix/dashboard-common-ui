...
→ **세우는 쪽이 없는 신호는 신호가 아니라 상수를 `var()` 로 감싼 것이다.** 읽는 쪽은 언제나 폴백(`block` / `none`)을 받는다. 그래서 두 이름을 지우고 읽는 쪽을 리터럴로 되돌렸다 — `ns-nav-group` 의 `.heading` 과 `ns-nav-item` 의 `.label`·`.trailing` 은 `display: block`, `.list.collapsed` 는 `display: none` 이다.
