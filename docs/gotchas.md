...
→ **되돌린 곳은 폭 토큰 하나다.** `--ns-sidebar-width-collapsed` 를 `4rem` 에서 `0` 으로 내렸고, 이름은 지우지 않았다 — 소비자가 이 값을 덮으면 좁은 띠가 남으므로, 레일 비슷한 것을 원하는 프로젝트가 설 자리가 그대로 있다. 라이브러리가 기본으로 강요하지 않을 뿐이다.
...
`ns-sidebar` 의 `<nav>` 가 이 컴포넌트의 유일한 랜드마크이고, 닫히면 호스트 폭이 0 이 되고 `overflow: hidden` 이 걸려 화면에서 사라진다. 0.4.0 은 접힘 상태에서도 항목이 배지로 남아 그 `nav` 안에 있었으므로 **이것은 0.5.0 이 잃은 것이다.**
