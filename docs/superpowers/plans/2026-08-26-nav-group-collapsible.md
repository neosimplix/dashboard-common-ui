...
`## B`(재현 불가, 소비자 프로젝트 필요)에 하나:

- **Next.js SSR 에서 `default-collapsed` 를 준 그룹이 첫 페인트부터 접혀 있는지.** 무엇이 잘못된 것인가: 펼쳐진 채로 그려졌다가 접히거나(깜빡임), 아예 펼쳐진 채로 남는 것. 후자면 `willUpdate` 씨앗이 동작하지 않는 것이다. `index.html` 은 순수 HTML 이라 이 경로를 재현하지 못하므로 `npm run check` 가 초록인 것은 아무 증거도 아니다.

`## A`(`index.html` 육안)에 여섯:
