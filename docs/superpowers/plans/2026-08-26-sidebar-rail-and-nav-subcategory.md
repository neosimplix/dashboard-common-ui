...
**기존 A 항목 중 셋이 낡았다. 그것을 먼저 처리한다.**

Run: `grep -n 'demo-sidebar\|레일에서 접힌' docs/pending-human-checks.md`

`A-7`·`A-8`·`A-9` 는 0.4.0 의 레일 모델을 서술한다 — "레일에서 접힌 그룹의 항목이 보이는가" 는 레일이 항목을 납작하게 나열하던 시절의 확인이고, 그 배선(`--ns-group-list-display`)은 이번에 제거됐다. 세 항목이 가리키는 `id="demo-sidebar-toggle"` 버튼도 Task 5 가 지웠다.

**세 항목을 지운다.** 확인할 수 없는 항목을 남기는 것은 목록을 훑고 넘기는 습관을 만들고, 그 습관이 다음 사이클의 진짜 새 항목까지 넘긴다 — 이 파일의 머리글이 그렇게 적고 있다. 지운 자리를 비워 두지 말고 아래 새 항목들이 번호를 이어받게 한다.

`A-10`(헤딩 버튼 포커스 링)·`A-11`(다크모드 헤딩 hover)은 **살아 있다** — 그룹 헤딩은 그대로 있고 이제 패널 안에 있을 뿐이다. 두 항목의 "볼 것" 이 가리키는 데모 위치만 Task 5 가 만든 새 절 이름으로 고친다.
...
- [ ] **Step 2b: `ns-sidebar` 절의 낡은 도입 문장을 고친다**

Task 5 가 그 절의 데모·표·동작 목록을 다시 썼지만 도입 `<p>` 하나가 남았다.

Run: `grep -n '배지로 항목을' index.html`

그 문장은 "접으면 좌측에 레일이 남아 **배지로 항목을** 계속 볼 수 있다" 로, 레일이 항목을 납작하게 나열하던 시절의 서술이다. 레일에는 이제 최상위 그룹 타일이 온다. 절 아래의 동작 목록이 이미 옳은 것을 말하고 있으므로 이 문장만 그것과 맞춘다.
