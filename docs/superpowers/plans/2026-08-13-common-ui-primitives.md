...
**정적 검증(구현자):** 특정도를 세어 보고서에 적는다. `.ns-checkbox .ns-checkbox__hint` 는 클래스 둘이라 (0,2,0), `.ns-checkbox span` 은 클래스 하나 + 타입 하나라 (0,1,1) 이다. 클래스 개수가 먼저 비교되므로 (0,2,0) 이 이긴다 — **순서와 무관하다.** CSS 를 옮겨 보는 실험은 하지 않는다. `npm run check` 는 CSS 를 평가하지 않으므로 아무것도 알려주지 않고, 결과는 화면으로만 드러난다.

**사람 확인:** hint 가 라벨보다 작고 옅게 보이는지. 같은 크기·같은 색으로 보이면 자손 선택자가 이긴 것이고, 그때가 이 규칙이 깨진 것이다.
