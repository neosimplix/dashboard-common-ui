...
- **`open` 의 제어/비제어 짝**(`open` 프로퍼티 전용 + `default-open`)과 `data-ns-open` 호스트 속성. 메뉴 토글이 소비자 배선 없이 동작하는 것은 레일이 없어도 이득이고, `docs/project-structure.md` 의 "남은 일" 을 절반 해소한 것도 그대로다.
