...
## 0. 레일을 걷어낸다 — 이 절이 §1~§6 을 뒤집는다

**아래 §1~§6 의 레일 설계는 만들어 보고 폐기했다.** 사용자가 화면을 보고 내린 판정이다 — 4rem 레일에 한 글자 타일이 쌓이니 **그것이 무엇인지 읽히지 않는다.** 아이콘을 준비하지 않은 소비자에게 타일은 「시」·「관」·「예」 같은 글자 하나이고, 그것으로 목적지를 고를 수 없다.

**그 문단들을 지우지 않고 남긴다.** 왜 그렇게 만들었고 왜 물렀는지가 다음에 같은 것을 다시 제안하는 것을 막는다. §1~§6 을 읽을 때는 이 절을 먼저 본다.
...
- **닫히면 사이드바가 통째로 사라진다.** 레일이 남지 않는다. 폭이 0 이 된다.
- **열리면 1차 카테고리가 전부 보인다.** 한 번에 하나가 아니다.
- **하위 카테고리는 눌러서 여는 그룹이다.** 이것이 원래 요청받은 기능이고 레일과 무관하게 살아남는다.

### 무엇이 죽나

레일과 함께 사라진다. **0.5.0 이 아직 나가지 않았으므로 breaking 이 아니다** — 이 이름들은 소비자에게 도달한 적이 없다.

| 죽는 것 | 있던 곳 |
|---|---|
| 타일 · `role="tablist"`/`tab`/`tabpanel` · 방향키 · roving tabindex | `ns-sidebar` |
| `slotAssignment: "manual"` 과 배정 코드 · `MutationObserver` | `ns-sidebar` |
| `activeGroup` · `default-active-group` · `ns-group-select` | `ns-sidebar` · `types.ts` · `elements.ts` · `Sidebar.tsx` |
| `data-ns-rail` | 소비자 훅 |
| `name` · `icon` · `badge` | `ns-nav-group` |
| `badge` | `ns-nav-item` — **사용자가 화면에서 지목한 것이다** |

### 무엇이 살아남나

- **하위 카테고리 중첩.** `#nested` 판정과 `.nested` 스타일 전부. 레일과 독립적이다.
- **`open` 의 제어/비제어 짝**(`open` 프로퍼티 전용 + `default-open`)과 `data-ns-open` 호스트 속성. **다만 비제어는 초깃값 전용이다.** 레일 타일이 이 컴포넌트 안에서 유일하게 토글을 요청하던 것이었고, 타일이 없어지면서 `ns-sidebar` 에는 자기를 여닫는 경로가 하나도 남지 않았다 — 그래서 `#toggled` 필드도 함께 지웠다. **`docs/project-structure.md` 의 "남은 일" 은 해소되지 않았다** — 여닫는 버튼은 여전히 `ns-header` 에만 있고 헤더는 자기가 누구를 여는지 모르므로, `ns-toggle` 을 받아 사이드바의 `open` 에 내려주는 한 줄이 소비자 코드로 남는다. 그 파일이 이 문장의 짝이고, 어긋나면 그쪽이 옳다.
  짝 자체를 남기는 이유는 다른 데 둘 있다 — ① HTML 의 초기 상태(`default-open`)와 React 의 제어 모드(`open`)가 한 이름을 겸용하지 않아야 하고(겸용하면 `<ns-sidebar open>` 이 제어 모드로 들어가 스스로 닫지 못한다), ② SSR 첫 페인트에 폭을 잡을 속성이 마크업에 남아 있어야 한다.
- **SSR 첫 페인트 방어 전부** — `willUpdate` 씨앗, `undefined` 좁히기, `default-open` 을 원시 속성으로 싣는 shim. 폭이 15rem ↔ 0 으로 바뀔 뿐 타임라인 문제는 같다.
- `--ns-label-display`·`--ns-group-list-display` 의 **삭제**. 되살리지 않는다 — 레일이 없으므로 라벨을 숨길 상황 자체가 없다.

### 폭

`--ns-sidebar-width` 는 **15rem** 으로 돌아간다. 뜻은 "열린 폭" 이다. `--ns-sidebar-width-collapsed` 는 **`0`** 이 된다 — 뜻이 "닫힌 폭" 그대로이고 값만 바뀐다. 이름을 지우지 않는 이유는 "닫혔을 때 본문을 얼마나 밀어 둘 것인가" 를 소비자가 정할 수 있어야 하기 때문이다.

**이 토큰을 덮어 레일을 되살릴 수는 없고, 되살릴 길을 두지도 않는다.** 값을 올리면 폭만 예약되고 안은 빈 채다. 이유가 둘이고 서로 독립이다 — ① 닫힘의 내용을 감추는 것이 폭이 아니라 상태다(`:host(:not([data-ns-open])) nav { visibility: hidden }` 는 닫힘 폭과 무관하게 걸린다), ② 그 규칙이 없더라도 좁은 띠를 읽히게 만들던 신호 프로퍼티 둘을 위에서 지웠고 되살리지 않으므로 띠에는 15rem 레이아웃의 잘린 왼쪽 끝이 보일 뿐이다. **탈출구를 만들지 않는 것이 이 절의 결론이다** — 준비물을 갖춘 소비자를 위한 옵트인 레일은 신호 프로퍼티를 되살리는 일이고, 그것은 위 판단을 뒤집는 별개의 결정이다.

### 항목의 생김새

**배지 사각형을 없앤다.** `ns-nav-item` 의 `badge` 는 레일이 항목을 납작하게 나열하던 시절 "레일에서 유일하게 남는 요소" 라는 이유로 있었고, 그 이유가 두 번 사라졌다(레일이 그룹 타일이 됐을 때 한 번, 레일이 없어지면서 또 한 번). 사용자가 화면에서 지목한 것이 이 사각형이다.

**`leading` 슬롯은 남는다.** 소비자가 원하는 요소를 넣는 자리다. 폴백이 없어졌으므로 **아무것도 넣지 않으면 그 자리가 접힌다** — 빈 사각형만큼 글자가 밀리지 않는다. `.leading` 래퍼를 없애고 `<slot name="leading">` 을 `.row` 의 직계로 둔다. 슬롯의 UA 기본값이 `display: contents` 라 배정된 것이 그대로 flex 항목이 되고, 배정된 것이 없으면 아무것도 차지하지 않으며 `gap` 도 생기지 않는다.

### 눌러지는 줄을 구분한다

토큰이 무채색 사다리이므로 "다른 색" 은 밝기 단계다. 세 단이 이미 있다.

| 자리 | 토큰 | 밝은 모드 |
|---|---|---|
| 그룹 제목 (누를 수 없다) | `--ns-color-fg-subtle` | 70.5% |
| 항목 링크 | `--ns-color-fg-body` | 37% |
| 활성 항목 | `--ns-color-fg` + 배경 | 21% |

여기에 **hover 에서 글자색이 `--ns-color-fg` 로 올라가는 반응**을 더한다. 지금은 hover 가 배경만 바꾸므로, 색이 함께 움직이면 "이 줄은 누를 수 있다" 가 정적인 대비뿐 아니라 반응으로도 드러난다. `collapsible` 인 그룹 제목은 이미 같은 모양의 hover 를 갖고 있어 둘이 같은 규약을 쓴다.
