...
`--ns-sidebar-width` 는 **15rem** 으로 돌아간다. 뜻은 "열린 폭" 이다. `--ns-sidebar-width-collapsed` 는 **`0`** 이 된다 — 뜻이 "닫힌 폭" 그대로이고 값만 바뀐다. 이름을 지우지 않는 이유는 소비자가 그것을 덮어 레일 비슷한 것을 원한다면 그 자리가 남아 있어야 하기 때문이다.
