...
네 번째는 **중복 id** 다. 각 페이지의 배선이 스크립트 요소 하나라 `getElementById` 가 엉뚱한 요소를 주는 순간 그 아래 전부가 실행되지 않는데, 화면은 멀쩡해 보인다. 새 절의 id 에는 절 이름을 접두사로 붙인다(`table-select-demo`).
