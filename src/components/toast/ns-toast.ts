 ...
 * 사이드바를 두므로 좌하단 토스트는 열린 사이드바에 그대로 가려진다 — 닫히면
 * 폭이 0 이라 겹칠 것이 없지만, 소비자가 여는 것이 사이드바의 정상 사용 상태이고
 * 열린 폭(15rem)이 화면 왼쪽을 채우는 그 상태 하나만으로도 뺄 이유가 된다.
 * 필요해지면 그때 더한다 — 값을 더하는 것은 breaking 이 아니다.
 */
