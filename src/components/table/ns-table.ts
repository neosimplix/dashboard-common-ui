  ...
  */
  ...
  /*
    ...
    ① 체크박스 — **`change` 가 아니라 여기서 처리하는 것이 중요하다.**
       ...
       라벨 클릭은 선택을 두 번 처리하지 않는다. `.ns-checkbox` 는 <label> 이
       <input> 을 감싸는 모양인데, 라벨의 click 은 타깃이 라벨이라
       closest() 가 비어 이 갈래에 걸리지 않고, 그 뒤 전달된 input 의 click
       하나만 처리된다.
    ...
    **타깃이 input 일 때 그렇다.**
    ...
    타깃이 라벨이면 이 순서가 지켜주지 못한다. `.ns-checkbox` 는
    `padding: var(--ns-space-1-5) 0` 과 gap 을 갖고 input 은 1rem 이라
    **라벨만 있는 클릭 영역이 실제로 존재한다.** 정렬 훅이 붙은 <th> 안에서
    그 영역을 누르면 체크박스 갈래의 closest() 가 비어 **정렬이 먼저 일어나고**,
    그 다음 라벨이 전달한 input 의 click 이 올라와 선택이 일어난다 — 한 번의
    클릭이 정렬과 선택을 둘 다 한다. (0.2.5 는 같은 마크업에서 ns-sort 를
    **두 번** 냈다. 라벨 타깃과 전달된 input 타깃이 각각 정렬 갈래에 걸렸다.
    지금은 한 번이다.)
