...
```json
"dependencies": {
  "@neosimplix/common-ui": "git+https://github.com/neosimplix/dashboard-common-ui.git#v0.6.0"
}
```
...
Dr. Reju-All 팔레트를 쓰려면 한 줄을 더한다. **`tokens.css` 다음에 온다** — 같은 `:root` 에 같은 이름을 다시 정의하는 것이 하는 일의 전부라 순서가 곧 결과다.

```css
@import "@neosimplix/common-ui/tokens.css";
@import "@neosimplix/common-ui/brand.css";     /* 이 한 줄 */
@import "@neosimplix/common-ui/controls.css";
```

**색 토큰 23 개만 덮는다.** 치수·타이포·radius 는 그대로다 — 브랜드와 무관하고 컴포넌트가 자기 여백 계산에 쓰는 값이라 바꾸면 레이아웃이 어긋난다.

**임포트하지 않으면 중립 팔레트 그대로다.** 브랜드가 아닌 화면(내부 도구·데모)은 부르지 않는다.

알아둘 것 둘. **라이트에서 `warn`·`success` 는 글자색으로 쓸 수 없다**(흰 배경 대비 2.16 · 2.56). 배지처럼 테두리·점·배경 조합으로 의미를 주고 글자는 본문색으로 둔다 — 다크에서는 한 단계 밝은 값을 써서 셋 다 통과한다. 그리고 **다크 절반은 브랜드북에 없던 값을 유도한 것**이라, 승인 절차가 있으면 확인이 필요하다. 근거와 대비 실측치는 파일 주석에 있다.
...
| 태그 | `dist/` | 소비자가 할 일 |
|---|---|---|
| [`v0.6.0`](./changelog.html#v0-6-0) | 변경 | **breaking 없음.** 브랜드 테마 파일 `brand.css` 가 늘었다 — 임포트하지 않으면 화면이 그대로다. 임포트하면 색 토큰 23 개가 Dr. Reju-All 팔레트로 바뀌고 치수·타이포는 그대로다. `tokens.css` 다음에 둔다. 다크 절반은 브랜드북에 없던 값을 유도한 것이라 승인 절차가 있으면 확인이 필요하다 |
| [`v0.5.8`](./changelog.html#v0-5-8) | 변경 | **breaking 둘.** `ns-table` 이 세로로 스크롤하지 않는다(`overflow-y: hidden`) — 높이를 제한한 자리에서는 스크롤 대신 잘린다. 그리고 확정 높이 flex 사슬에서 표가 눌리지 않고(`flex-shrink: 0`) 조상이 넘친다. 되돌리려면 `@layer` 밖 규칙으로 덮는다. Safari 에서 모든 표가 유령 세로 스크롤바를 갖던 것을 고친 것이다 — 원인은 `overflow-x` 만 쓰면 세로축이 `auto` 로 계산되는 것과, WebKit 이 flex·grid 항목에 가로 스크롤바 두께를 더하지 않는 것의 합이다. `overflow-x: scroll` 은 넘침 자체를 없애지만 모든 표에 빈 막대를 남겨 고르지 않았다 |
| [`v0.5.7`](./changelog.html#v0-5-7) | 변경 | **breaking 하나(`ns-pagination` 번호 버튼의 접근 가능한 이름이 `"3"` → `"3페이지"` 로 바뀐다 — 정확히 일치하는 문자열로 집던 테스트가 깨질 수 있다).** 그 밖에 `ns-pagination` 은 범위 밖 `page` 의 clamp 책임(제어 모드는 소비자 몫)에 맞춰 제어 모드 경고 문구를 바꾸고, 경고가 인스턴스당 한 번이라는 것과 폭 고정이 소비자 글꼴의 `tabular-nums` 지원에 달렸다는 것을 문서로 밝힌다. 무관하게 `Field` 는 RSC 경로(자식이 서버에서 `React.lazy` 참조로 도착)에서도 이제 경고한다 — 서버 경고는 `next dev` 터미널, 기존 클라이언트 경고는 브라우저 콘솔이지만 SSR 앱에서는 `null`·배열·문자열 같은 비-엘리먼트 자식이 둘 다에 함께 뜬다. `v0.5.6` 의 경고를 옮긴 것이 아니라 자리를 하나 더한 것이고, 프롭을 삼키는 래퍼는 여전히 잡지 못한다 |
