...
| 문서 페이지 육안 확인 | 렌더·상호작용. `guide.html` 과 `changelog.html` 의 셸이 이 라이브러리로 만들어져 있어 깨지면 문서가 안 열린다. `index.html` 은 링크 둘뿐인 목차라 이 수단의 대상이 아니다 |
...
루트의 문서 HTML 셋(`index.html`·`guide.html`·`changelog.html`) 중 어느 것이든
고친 뒤 브라우저 없이 할 수 있는 검사:
...
**기대값이 파일마다 다르다.** `index.html` 은 목차라 배선이 없고, 그래서 첫 검사가
0 이어야 한다 — 이것은 빠진 것이 아니라 **바** 다. 1 이 나오면 둘 중 하나다.
누군가 필요 없는 스크립트를 더했거나, 산문에 스크립트 태그를 리터럴로 적었다.

**첫 검사는 파싱이 아니라 그냥 문자열 세기다.** 그래서 산문이 그 태그 이름을
리터럴로 적으면 배선 개수로 잡혀 검사가 거짓이 된다. `changelog.html` 이 실제로
여기에 걸렸고 문구를 고쳐 풀었다. 세 페이지의 주석과 산문에서는 그 태그를
리터럴로 쓰지 않는다 — "스크립트", "스타일 블록" 처럼 우리말로 적는다.
