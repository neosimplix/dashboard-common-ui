...
- **`Field` 와 조합해야 하는 커스텀 엘리먼트 컨트롤의 래퍼는 정적 `nsFieldControl` 마커를 갖는다.** `Field` 가 주입해야 할 프로퍼티 이름(`id`·`describedby`·`invalid`)을 담는다. `Field` 는 래퍼를 import 하지 않고 `element.type.nsFieldControl` 로만 읽는다. 마커는 정의하는 자리에서 `satisfies Record<"id" | "describedby" | "invalid", keyof …Element>` 로 값을 제약한다 — 존재하지 않는 프로퍼티 이름이면 여기서 컴파일이 실패한다.
