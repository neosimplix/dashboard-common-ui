...
- **`key` 를 속성/프로퍼티 이름으로 쓰지 않는다.** React 가 재조정 키로 소비해 엘리먼트까지 도달하지 않고, shim 으로도 고칠 수 없다 — `title` 은 우리에게 도착한 뒤 이름을 바꿀 수 있었지만 `key` 는 도착하지 않는다. 키는 `name` 이다. (`ns-nav-group` 의 `name` 이 `ns-sidebar` 레일의 키다)
...
- **컴포넌트 유형이 셋이다.** shadow + 렌더(기본), Light DOM + 렌더(`ns-pagination`), Light DOM + 렌더 없음(`ns-table`). 판단은 "캡슐화가 필요한가 / 소비자 자식을 품는가" 두 축이다. **유형은 늘지 않았지만 성질 하나가 늘었다** — `ns-sidebar` 는 여전히 shadow + 렌더이면서 `slotAssignment: "manual"` 로 **소비자 자식을 골라 배정한다.** 배정하지 않은 자식은 숨는 것이 아니라 렌더되지 않고, 그 대가로 그 컴포넌트에서는 표준 `slot` 속성이 아무 의미가 없다.
...
- **소비자 DOM 의 속성을 관리하면 `MutationObserver` 가 필요하다.** `updated()` 는 반응형 프로퍼티 변경만 본다. `{ childList: true, subtree: true }` 를 쓰고 **`attributes` 는 켜지 않는다** — 동기화가 `setAttribute` 를 쓰므로 자기 쓰기에 재발동해 루프가 된다. **`attributeFilter` 가 우리가 쓰지 않는 이름들로 좁혀져 있으면 켜도 된다** — 재발동 경로가 없어져 규칙이 막으려던 것이 남지 않는다. `ns-sidebar` 가 그 경우다: 동기화가 `slot.assign()` 이라 자식의 속성을 쓰지 않고, 필터가 `name`·`heading`·`icon`·`badge`·`data-ns-rail` 다섯으로 못박혀 있다. 그때 `subtree` 는 함께 켠다 — `attributes` 는 관찰 대상 노드 **자신의** 속성만 보므로 없으면 자식의 속성 변화를 놓친다.
...
- **컴포넌트 스타일에 `var()` 폴백을 쓰지 않는다.** 색·치수는 `tokens.css` 한 곳에만 존재한다. **예외는 신호 프로퍼티다** — 정의가 `tokens.css` 에 없고 세우는 쪽이 없는 상황에서는 폴백이 곧 기본 동작이라, 지우면 배선이 끊긴다. **지금 그 예외에 해당하는 이름은 하나도 없다.** 0.4.0 의 `--ns-label-display`·`--ns-group-list-display` 둘은 레일이 항목을 그리지 않게 되면서 세우는 쪽이 사라져 0.5.0 에 함께 죽었다. 규칙은 그대로 남는다 — 다음에 신호가 생기면 같은 근거로 예외가 된다. 목록의 출처는 여전히 `scripts/check-tokens.mjs` 의 `WIRING` 집합이고, 그 집합은 지금 **의도적으로 빈 채로** 있다.
