/*
  ...
  **대상은 DOC_PAGES 의 문서 페이지 전부다.** 셋 다 <style> 안에서 같은
  light-dark(var(--ns-…), var(--ns-…)) 짝과 토큰을 쓰므로, 리터럴 색이 끼어들
  자리가 셋에 똑같이 있다.

  ①②를 켜지 못하는 이유는 셋 중 둘에만 있다 — guide.html 과 changelog.html 은
  산문에서 옛 무접두사 이름을 일부러 적는다(changelog.html 의 v0.2.0 절이 접두사
  도입을 `var(--space-3)` → `var(--ns-space-3)` 로 설명한다). 링크 둘만 있는
  index.html 에는 그런 산문이 없어 원리상 ①을 켤 수 있지만 켜지 않는다.
  ①②의 대상은 src 아래의 코드이고, 문서 HTML 은 페이지 단위가 아니라 **집합
  단위로** 그 밖에 있다. 페이지마다 "이건 산문이 있나" 를 따져 목록을 갈라 두면
  그 답이 페이지를 고칠 때마다 바뀌고, 어느 쪽에 들어가는지를 아무도 다시 보지
  않는다. index.html 의 면제는 스스로 얻은 것이 아니라 집합에서 물려받은 것이다.

  그러니 "산문이라 ①을 켤 수 없고, <style> 안은 산문이 아니라 ⑤를 켤 수 있다" 는
  판단이 세 파일에 똑같이 적용된다 — 범위를 넓혀도 이 규칙이 <style> 블록에
  갇혀 있는 이유는 그대로다. 루트에 문서 HTML 을 더하면 이 목록에 넣는다.
  ...
  한계:
    - 참조가 규칙을 지키는지만 본다. 그 토큰이 화면에서 옳은 값인지는
      guide.html 육안 확인의 몫이다.
    - 문서 페이지(DOC_PAGES)는 ①②의 검사 대상이 아니다. 세 파일 다 배포되고(README 가
...
/*
  ...
  **지금 비어 있다.** 0.4.0 에는 둘이 있었다 — ns-sidebar 가 ::slotted 로
  자식에게 접힘 상태를 알리던 신호 프로퍼티 둘(라벨 표시·리스트 표시)이다.
  세우는 쪽은 두 걸음에 걸쳐 사라졌다. 0.5.0 개발 중에 레일을 최상위 그룹 타일만
  갖는 모델로 다시 만들면서 레일에 항목이 없어져 신호의 전제가 먼저 없어졌고,
  그 레일이 화면에서 무엇인지 읽히지 않아 물렀으므로 **지금은 레일 자체가 없다** —
  닫히면 사이드바가 통째로 사라진다. 세우는 쪽이 없는 신호는 읽는 쪽이 언제나
  폴백을 받으므로 신호가 아니라 상수를 var() 로 감싼 것이 된다. 경위는
...
/*
  루트의 문서 HTML 전부. 셋 다 package.json 의 files 에 들어 배포되고, 셋 다
  자기 <style> 에서 토큰을 참조한다. 파일을 더하면 여기에 넣는다.

  index.html 은 guide.html 과 changelog.html 로 보내는 목차 페이지다. 링크
  둘뿐이라 <style> 도 짧지만, 짧다고 빼면 거기 들어간 리터럴 색만 검사 밖에
  남는다 — 크기가 아니라 "배포되는 문서 페이지인가" 가 이 목록의 기준이다.
*/
const DOC_PAGES = ["index.html", "guide.html", "changelog.html"];
