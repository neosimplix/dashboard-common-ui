...
fe = function() {
  ...
  return l(this, et) || (r(this, et, !0), console.warn(
    l(this, g, pe) ? `[ns-pagination] page=${t} 가 1..${s} 범위를 벗어났습니다. 표시용으로만 ${e} 로 보정하고 이벤트로 교정하지는 않습니다 — 소비자가 page 를 1..${s} 로 clamp 해서 넘겨야 합니다.` : `[ns-pagination] 현재 페이지 ${t} 가 total=${this.total} · per-page=${this.perPage} 로 계산된 페이지 수(${s})를 벗어났습니다. 표시용으로 ${e} 로 보정합니다.`
  )), e;
}, /** 이동했으면(= 이벤트를 냈으면) true. */
Ye = function(t) {
